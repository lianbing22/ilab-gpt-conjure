...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_sidebar_footer_utilities_are_centered(self) -> None:
        ...
        self.assertIn('aria-controls="versionModal"', html)
        self.assertIn('aria-expanded="false"', html)
        self.assertIn('id="versionReleaseHistory"', html)
        ...
        self.assertRegex(styles, r"\.version-popover-panel\s*\{[^}]*position:\s*fixed")
        self.assertRegex(styles, r"\.version-popover-panel\s*\{[^}]*width:\s*min\(440px,\s*calc\(100vw\s*-\s*24px\)\)")
        self.assertRegex(styles, r"@media\s*\(max-width:\s*520px\)[\s\S]*?\.version-popover-panel\s*\{[^}]*max-height:\s*72dvh")
        self.assertRegex(styles, r"\.version-release-item:not\(:last-child\)::before\s*\{[^}]*background:\s*var\(--line\)")
    ...
    def test_app_version_status_uses_api_and_updater_action(self) -> None:
        ...
        self.assertIn("release_history", script)
        self.assertIn("version.change.", script)
        self.assertIn("function positionVersionPopover", script)
        self.assertIn("getBoundingClientRect", script)
        self.assertIn("availableRight >= 300", script)
        self.assertIn('matchMedia("(max-width: 520px)")', script)
